  ...
  const setItemsHandle = () => {
    setItems([
      ...
      },
      {
        ...
        children: <div key="1" className={styles.taskList}>
      ...
      },
      {
        ...
        children: 
        <form key="4" className={styles.formAddTask}>
          <ConfigProvider
            theme={{
              components:{
                Select:{
          ...
          <Select style={{ width: '100%' }} placeholder='Исполнитель' showSearch filterOption={filterOptionCreate} className={styles.fromChild} options={projectData?.users?.map(user => user)}  onChange={selectedValues => {
        ...
        <Layout className={styles.projectMain}>
          <Sider width={250} className={styles.siderContainer}>
            <ul className={styles.userList}>
              <li key="link"> <Link href={`/${username}`}>{arrowBack}</Link> <h1>{projectData?.name}</h1></li>
              <li key="members">
                <h1>Участники</h1>
                <button onClick={() => handleToggleOpenModal()}>+</button>
              </li>
              { projectData?.users.map(el => <li key={el.id} className={styles.element}>{el?.label}</li>) }
            </ul>
          </Sider>
