...
export default function UserProjects() {
  ...
  useEffect(() => {
    ProjectsAPI.getUserProjects().then(e => setProjects(e.data))
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      ...
      .then(async () => ProjectsAPI.getUserProjects().then(e => setProjects(e.data)))
    }
  })
      ...
      <div className={`container`}>
        <React.Suspense fallback={<Loader />}>
          <div className={styles.main}>
            {projects?.map((el) => {
              if (el.creator === username || el.users.some((user:any) => user.value === username))
