...
export default function Chat() {
  ...
  return (
    <div className={'chatContainer element'} onLoad={setFocus()}>
      <Helmet>
      ...
      <div className='content'>
        ...
        {messages?.sort((el, el1) => compareAsc(new Date(el.date), new Date(el1.date)))?.map(mes => (
            <div key={mes.id}>
                <div className={`message ${mes.userfrom === user.login ? 'usFr' : ''}`}>
                  ...
                  <h6 className='senderDate'>{new Date(mes.date).getHours() < 10? 0: ''}{new Date(mes.date).getHours()}:{new Date(mes.date).getMinutes() < 10? 0: ''}{new Date(mes.date).getMinutes()}</h6>
                </div>
            </div>
          ))}
